...
위 예의 괄호 표현은 그림 위에 주어져 있다. 쇠막대기는 레이저에 의해 몇 개의 조각으로 잘려지는데,
...
# 여는괄호 -> 일단 stack.push('(')
# 닫는괄호 -> 일단 stack.pop() 하고
#           - 이전 문자가 여는 괄호였다면 (레이저인 경우) -> 조각 수(cnt) += 여는괄호 수(len(stack))
#           - 이전 문자가 여는 괄호가 아니라면 (막대기의 끝인 경우) -> 조각 수(cnt) += 1

s = input()
stack = [] # 여는 괄호 '(' 를 관리할 스택
cnt = 0 # 잘린 조각의 총 개수

for i in range(len(s)):
    if s[i] == '(': # 여는괄호 -> 일단 stack.append('(')
        stack.append(s[i])
    else: # 닫는괄호 -> 일단 stack.pop()
        stack.pop()
        if s[i-1] == '(':
            cnt += len(stack)
        else:
